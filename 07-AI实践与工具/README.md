...
> 🎯 **新手建议**：直接学 PyTorch。目前绝大多数教程、课程、开源项目都以 PyTorch 为主。2024 年 PyTorch 市场占有率超过 80%，已成为事实标准。Apple Silicon 用户也可关注 **MLX** 框架。
...
- **LangChain / LangGraph**：LangChain 是构建 LLM 应用的核心框架，支持链式调用（Chains）、工具使用（Agents）、记忆管理（Memory）、RAG 等。2024 年推出的 **LangGraph** 专注于构建复杂的 Agent 工作流，支持状态管理和循环执行。配套工具包括 LangSmith（调试监控）。
- **LlamaIndex**：专注于**数据连接**——把你的私有数据（PDF、数据库、API）接入 LLM，擅长构建知识问答系统。
- **DSPy**：斯坦福推出的结构化提示框架，通过编程方式定义和优化提示，比手工 Prompt Engineering 更系统化、可复现。
...
| 服务商 | 代表模型 | 特点 |
|---|---|---|
| **OpenAI** | GPT-4o、GPT-4o mini、o1/o3 | 综合能力强，生态最完善 |
| **Anthropic** | Claude 3.5 Sonnet、Claude 3.5 Haiku | 长上下文（200K）、安全性好、代码能力突出 |
| **Google** | Gemini 2.0、Gemini 1.5 Pro | 超长上下文（100 万 token）、多模态、视频理解 |
| **DeepSeek** | DeepSeek-V3、DeepSeek-R1 | 开源模型 API，推理能力强，成本极低（仅为 GPT-4 的 1/10~1/20） |
...
| **Qdrant** | Rust 编写，高性能，支持混合搜索，2024 年增长最快 |
| **Milvus** | 开源、高性能、支持十亿级向量 |
| **Weaviate** | 支持混合搜索（向量 + 关键词），生产级 |
| **Pinecone** | 全托管云服务、零运维 |
| **FAISS** | Meta 开源、本地使用、速度极快 |

> 💡 **2024 最佳实践**：混合搜索（结合向量检索和关键词检索 BM25）已成为 RAG 系统的标准配置，可显著提升检索质量。
...
- **合成数据**：使用 LLM 生成训练数据已成为 2024 年的主流做法。典型应用包括：用 GPT-4/Claude 生成指令微调数据、用 LLM 进行数据标注和质量评估。高质量合成数据可以有效替代部分人工标注数据。
- **偏好数据收集**：对于 DPO 等对齐方法，需要收集（好回答, 差回答）成对偏好数据。
...
第 1 阶段（1-2 周）：Python 基础 + NumPy/Pandas 数据处理
        ↓
第 2 阶段（1-2 周）：Prompt Engineering + LLM API 应用（快速上手 AI）
        ↓
第 3 阶段（2-3 周）：RAG 系统构建（LangChain/LlamaIndex + 向量数据库）
        ↓
第 4 阶段（2-3 周）：Scikit-learn 经典机器学习 + PyTorch 深度学习入门
        ↓
第 5 阶段（2-3 周）：模型微调（LoRA/DPO）+ Agent 系统开发（LangGraph）
        ↓
第 6 阶段（持续）：选择方向深入（多模态 / 推理模型 / MLOps / 具身智能）
```

> 💡 **2024 调整**：相比传统从理论到应用的路径，现在更推荐"先用后学"——先通过 API 和框架快速构建 AI 应用，建立直觉后再深入学习底层原理。
